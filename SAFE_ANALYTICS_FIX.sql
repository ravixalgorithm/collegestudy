-- ============================================
-- SAFE ANALYTICS FIX SCRIPT
-- ============================================
-- This script safely handles function signature conflicts
-- by dropping existing functions before recreating them
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== SAFE ANALYTICS FIX COMPLETED ===';
    RAISE NOTICE '';

    -- Test download analytics
    SELECT * INTO analytics_result FROM get_download_analytics() LIMIT 1;
    RAISE NOTICE 'Download Analytics: % total downloads', analytics_result.total_downloads;

    -- Test popular notes
    SELECT COUNT(*) INTO popular_count FROM get_popular_notes(5);
    RAISE NOTICE 'Popular Notes: Found % notes', popular_count;
...
-- Show sample results
SELECT '=== SAMPLE ANALYTICS RESULTS ===' as info;

SELECT 'Download Analytics:' as type, * FROM get_download_analytics();

SELECT 'Popular Notes:' as type, title, download_count
FROM get_popular_notes(3)
ORDER BY download_count DESC;

SELECT 'Branch Performance:' as type, name, user_count, notes_count
FROM get_branch_analytics()
...
-- ============================================
SELECT
    '🎉 ANALYTICS DASHBOARD READY!' as status,
    'Go to your admin dashboard and click "Analytics" in the sidebar' as next_step;
