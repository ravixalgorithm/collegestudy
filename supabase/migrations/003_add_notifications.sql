...
-- Replace forum functionality with comprehensive notifications
-- Date: 2024-12-19
...
CREATE INDEX idx_notifications_is_published ON notifications(is_published);
CREATE INDEX idx_notifications_target_branches ON notifications USING GIN(target_branches);
CREATE INDEX idx_notifications_target_semesters ON notifications USING GIN(target_semesters);
...
-- ============================================
-- EXAM REMINDER FUNCTION (to be called by cron job)
-- ============================================
...
BEGIN
    ...
    -- Same day reminders (2 hours before exam)
    FOR exam_record IN
        SELECT e.*, s.name as subject_name, s.code as subject_code
        FROM exam_schedule e
        JOIN subjects s ON e.subject_id = s.id
        WHERE e.exam_date = CURRENT_DATE
          AND e.start_time > CURRENT_TIME + INTERVAL '2 hours'
          AND e.start_time < CURRENT_TIME + INTERVAL '2 hours 30 minutes'
    LOOP
        SELECT create_and_deliver_notification(
            '🚨 Exam Starting Soon: ' || exam_record.subject_code,
            'Your ' || exam_record.exam_type || ' exam for ' || exam_record.subject_name ||
            ' starts at ' || exam_record.start_time::TEXT ||
            ' in Room ' || COALESCE(exam_record.room_number, 'TBA') || '. Get ready!',
            'exam_reminder',
            'urgent',
            FALSE,
            ARRAY[exam_record.branch_id],
            ...
            'exam',
            exam_record.id,
            jsonb_build_object('exam_id', exam_record.id, 'hours_until', 2)
        ) INTO reminder_count;
    END LOOP;
